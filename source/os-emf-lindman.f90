!
! $URL: https://osiris.ist.utl.pt/svn/branches/dev_3.0/source/os-emf-lindman.f90 $
! $Id: os-emf-lindman.f90 557 2013-04-30 13:11:39Z zamb $
!
...
interface update_emfbound
  module procedure update_emfbound_lindmann
end interface
...
!---------------------------------------------------
 ...
 x_dim   = space_dim(b)
 lgc_num(:,1:x_dim) = b%gc_num(:,1:x_dim)
 lnx(1:x_dim) = b%nx(1:x_dim)
 
 ! set simulation parameters
 this%dtdxi(1:x_dim) = real( dt /b%dx(1:x_dim), p_k_fld ) 
 this%disp_corr = real( (1.0_p_double - dt/b%dx(i_dim))/ &
						(1.0_p_double + dt/b%dx(i_dim)), p_k_fld )
 ...
 i_bounds(2,1:x_dim)=b%nx(1:x_dim)
...
!---------------------------------------------------
subroutine update_boundary_lindman(this, no_co, nx_move)
!---------------------------------------------------
   implicit none
   ...
   if (this%idir > 0) then
	  call update_boundary(this%b_buffer, p_vdf_replace, no_co, nx_move)
	  call update_boundary(this%e_buffer, p_vdf_replace, no_co, nx_move)
   end if
...
case (p_upper)
  ! no update required for upper lindmann
  ! do nothing
...
!---------------------------------------------------
subroutine update_emfbound_lindmann( this, b, e, dim )
!---------------------------------------------------
!
...
end subroutine update_emfbound_lindmann
!---------------------------------------------------
...
!---------------------------------------------------
subroutine reshape_lind( this, old_lb, new_lb, no_co )
!---------------------------------------------------
  implicit none
  ...
  call reshape_copy( this%b_buffer, old_lb, new_lb, no_co )
  call reshape_copy( this%e_buffer, old_lb, new_lb, no_co )
