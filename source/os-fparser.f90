!
! $URL: https://osiris.ist.utl.pt/svn/branches/dev_3.0/source/os-fparser.f90 $
! $Id: os-fparser.f90 558 2013-04-30 16:28:11Z zamb $
!
!
...
! restrict access to things explicitly declared public
  ...
  ! operators are specified with increasing prioryty
  ! logical operators && and || have the lowest priority
  ! next are comparation operators ( ==, >= , !=, etc.)
  ! finally come the arithmetic operators
  !
  ! note that if two operators begin with the same character
  ! and have different lengths
  ! the longest one MUST be specified first because of the
  ! implementation of the function if_operator
  !
  ! operator ^ is used for a ^ int(b) i.e. b is converted to an integer
  ! before evaluating. If you want b to not be an integer use the function
  ! pow(a,b) but if a < 0 pow returns NaN
  
  ! NOTE: Operators must not have alphanumeric characters or dots
  
  character (len = MAX_OP_LEN), dimension(n_oper), parameter ::  oper = &
                                (/ '&&', '||', '==', '!=', '>=', '<=', &
                                   '> ', '< ',  &
                                   '+ ', '- ', '* ', '/ ', '^ ' &
                                    /)
...
!-----------------------------------------------------------------------------------------    
  ...
  integer :: opidx, op_len          ! operator index, operator length
  ...
  do opidx = 1, n_oper  ! parse operators starting from lowest priority
    
    k = 0
    
    ! Process string from end to beginning
    do i=sub_str_len,1,-1  
      ...
      if ((k == 0) .and. (opidx == is_operator(sub_str(i:), op_len))) then
         
         if ((i==1) .and. ((sub_str(1:1)=='-') .or. (sub_str(1:1)=='+'))) then
             ! variable or constant beggining with + or -
             ! ignore
             continue
         elseif ( num_signed_exp( i, sub_str ) ) then  
...
parse:  do
      ...
      if (scan(c, '0123456789.') > 0) then  ! number
        !r = strtodouble(func(i:), ierr)      
...
!     err_point = " "
     ...
     write(0,*) "Error compiling function:"
     write(0,*) in_str
     write(0,*) repeat(' ',pos-1) // '^'
     ...
     select case (ierr)
       ...
       case ( err_param )
         write(err_msg,*) "Invalid number of parameters, ",local_nparam," expected"
       case default
         err_msg =  "Syntax error"
     end select
     
     write(0,*) trim(err_msg)//", col = ", pos
...
!-----------------------------------------------------------------------------------------
! checks if in_str begins with an operator
!-----------------------------------------------------------------------------------------
