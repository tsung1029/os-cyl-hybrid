...
!--------------------------------------------------------------------------------------------------
  ...
  integer, dimension(:,:), intent(in) :: ave_nx_range ! range of averaged grid being
  															! processed
...
!--------------------------------------------------------------------------------------------------
  ...
  real( p_single ), dimension(:), intent(inout) :: buffer	! buffer to save data to
  ...
  if ( envelope ) then
	...
	if (  mod_ave(1)  == 0 ) then
	   ...
	   do gi1 = grange( p_lower, 1 ), grange( p_upper, 1 )
		  ...
		  tmp = abs( real ( fld( fcomp, gi1 - nx_p( p_lower, 1 ) + 1), p_single ) )
	...
	if (  mod_ave(1)  == 0 ) then
	   ...
	   do gi1 = grange( p_lower, 1 ), grange( p_upper, 1 )
		  ...
		  buffer(idx) = buffer(idx) + real ( rdx_ave(1) * &
										fld( fcomp, gi1 - nx_p( p_lower, 1 ) + 1), p_single )
	   enddo
	else
	   do gi1 = grange( p_lower, 1 ), grange( p_upper, 1 )
		  ai1 = int( (gi1-1) * rdx_ave(1) ) + 1
...
!--------------------------------------------------------------------------------------------------
  ...
  type( t_grid ), intent(in)        :: grid     ! global grid info
  ...
  real(p_single), dimension(:), pointer :: buffer_full, buffer1, buffer2
  integer :: ierr
  
  ! hdf variables
  type(t_diag_file) :: diagFile
  integer(hsize_t), dimension(3) :: dimsFile
  integer(hsize_t), dimension(3) :: start, count, block, stride
  integer(hid_t) :: filespaceID, memspaceID, datasetID
 
  ! variables to check hdf5 version
!  integer :: majnum, minnum, relnum
!  character( len = 8 ) :: h5version
  ...
  if ( nchunks > ave_nx( vdf%x_dim ) ) then
ERROR('averaged grid too large')
ERROR('n_ave = ', n_ave(1:vdf%x_dim))
ERROR('try increasing n_ave or increasing diag. buffer size')
		call abort_program( p_err_alloc )
  endif
  chunksize = ave_nx( vdf%x_dim ) / nchunks
  
  ! create file and dataset
  if (root( no_co )) then 
  
    ! Create the report file
    call create_report_file(vdf, report, g_space, grid, no_co, diagFile, single_node = .true. )
    
    ! create dataset
	dimsFile( 1 : vdf%x_dim ) = ave_nx( 1 : vdf%x_dim )
	call h5screate_simple_f( vdf%x_dim, dimsFile, filespaceID, ierr ) 
	call h5dcreate_f( diagFile%id, report%name, H5T_NATIVE_REAL, filespaceID, datasetID, ierr )
	call h5sclose_f(filespaceID, ierr)
	
	! create dataset attributes
	call add_h5_atribute( datasetID, 'UNITS', report%units)
	call add_h5_atribute( datasetID, 'LONG_NAME', report%label )

  endif    

  lnx_p(:,1:vdf%x_dim) = grid%my_nx(:,1:vdf%x_dim)
	...
	case (1) ! --------------------------------------------------- 1D data ----
	  ...
	  do 
        ...
        if (root( no_co )) then
          ...
          if ( ldatasize > 0 ) then
			 call average1D( vdf%f1, fcomp, buffer2, global_nx, lnx_p, ave_nx, local_ave_nx, &
			                 envelope )
          ...
          do node = 2, no_num( no_co )
             ...
             if (  rdatasize > 0 ) then
                ! post receive
                call mpi_irecv( buffer2, bsize, MPI_REAL, mpi_rnode, 0, comm(no_co), request, ierr )
             endif
...
! temporary fix because running out of buffer space on some systems
		  ...
		  call h5screate_simple_f(1, block, memspaceID, ierr)
          ...
          if ( ldatasize > 0 ) then
             ! get average values
			 call average1D( vdf%f1, fcomp, buffer1, global_nx, lnx_p, ave_nx, local_ave_nx, &
			                 envelope )
             ...
             ! send to node 0
             call mpi_send( buffer1, ldatasize, MPI_REAL, 0, 0, comm( no_co ), ierr )
	...
	case (2) ! --------------------------------------------------- 2D data ----
	  ...
	  do 
        ...
        if (root( no_co )) then
          ...
          ! take care of local data if any
          if ( ldatasize > 0 ) then
			 call average2D( vdf%f2, fcomp, buffer2, global_nx, lnx_p, ave_nx, local_ave_nx, &
			                 envelope )
...
! temporary fix because running out of buffer space on some systems
		  ...
		  ! hyperslab coordinates are 0 indexed
		  start(1:2)  = (/ 1, chunk(1) /) - 1
		  block(1:2)  = (/ ave_nx(1), chunk(2) - chunk(1) + 1/)
		  count(1:2)  = 1
		  stride(1:2) = 1
		  
		  dimsFile(1:2) = ave_nx(1:2)
                    
		  ! create memory dataspace
		  call h5screate_simple_f(2, block, memspaceID, ierr)
          ...
          if ( ldatasize > 0 ) then
             ! get average values
			 call average2D( vdf%f2, fcomp, buffer1, global_nx, lnx_p, ave_nx, local_ave_nx, &
			                 envelope )
	...
	case (3) ! --------------------------------------------------- 3D data ----
	  ...
	  do 
        ...
        if (root( no_co )) then
          ...
          ! take care of local data if any
          if ( ldatasize > 0 ) then
			 call average3D( vdf%f3, fcomp, buffer2, global_nx, lnx_p, ave_nx, local_ave_nx, &
			                 envelope )
...
! temporary fix because running out of buffer space on some systems
		  ...
		  ! hyperslab coordinates are 0 indexed
		  start(1:3)  = (/ 1, 1, chunk(1) /) - 1
		  block(1:3)  = (/ ave_nx(1), ave_nx(2), chunk(2) - chunk(1) + 1/)
		  count(1:3)  = 1
		  stride(1:3) = 1
		  
		  dimsFile(1:3) = ave_nx(1:3)
                    
		  ! create memory dataspace
		  call h5screate_simple_f(3, block, memspaceID, ierr)
          ...
          if ( ldatasize > 0 ) then
             ! get average values
			 call average3D( vdf%f3, fcomp, buffer1, global_nx, lnx_p, ave_nx, local_ave_nx, &
			                 envelope )
