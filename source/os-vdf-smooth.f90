...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
! $URL: https://osiris.ist.utl.pt/svn/branches/dev_3.0/source/os-vdf-smooth.f90 $
! $Id: os-vdf-smooth.f90 484 2012-10-02 15:00:23Z zamb $
!
...
!-----------------------------------------------------------------------------------------
subroutine read_nml_smooth( this, input_file )
!-----------------------------------------------------------------------------------------
...
!-----------------------------------------------------------------------------------------
  ...
  type( t_vdf),     intent(inout) :: pf
  type( t_smooth ), intent(in)    :: this

  select case ( pf%x_dim )
   case (1)
   ...
   case default
	 ERROR("invalid dimensions on pf vdf, xdim = ",space_dim(pf))
...
!-----------------------------------------------------------------------------------------
  ...
  real(p_k_fld), dimension( pf%f_dim, -max_order:max_order ) :: o
  real(p_k_fld), dimension( pf%f_dim ) :: f
