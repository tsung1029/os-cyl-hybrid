...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
! $URL: svn+ssh://exppmaster.ist.utl.pt/svn_repositories/osiris/trunk/source/os-rest.f90 $
! $Id: os-rest.f90 169 2008-04-11 18:23:36Z samuel $
...
!  use m_system
  ...
  character(len=*), parameter :: p_rest_rst_id = "restart rst data - 0x0001"
  ...
  type  :: t_restart_handle
	...
	integer(selected_int_kind(10))  ::  data_size
...
!         prepare path and file names
  ...
  if ( n /= this%debug_iter ) then
	 ...
	 if (ndump <= 0) then 
	   ...
	   ! number restart files sequentially
...
!---------------------------------------------------
subroutine restart_write_close( this, gcom, ngp_id, n, ndump, restart_handle )
!---------------------------------------------------
!  close newly written restart files and make them accessable
!---------------------------------------------------
...
!       local variables
  logical   :: remold
  character(len = 80)   :: target, name, oldfile
  integer         :: ierr

!       executable statements

  remold = ( (n /= this%debug_iter) .and. this%if_remold .and. &
           (restart_handle%old_file_name /= '' ))
  ...
  target    = restart_handle%current_file_name

  #if defined( __RST_BINARY__)
    name      = trim(path_rest) // 'rstrtdat' // '.n' // idx_string( ngp_id, p_node_length )
    call link( trim(target), trim(name), ierr )
  #elif defined( __RST_SION__)
    if ( ngp_id == 1 ) then
  ...
  if( remold ) then
    ! remove old file
    oldfile = trim(path_rest) // trim(restart_handle%old_file_name)
    call remove(trim(oldfile),ierr)
  endif
  restart_handle%old_file_name = restart_handle%current_file_name
...
!---------------------------------------------------
! include i/o procedures for the apropriate file backend
!---------------------------------------------------
